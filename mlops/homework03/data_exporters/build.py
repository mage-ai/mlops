...
from mlops.homework03.utils.data_preparation.encoders import vectorize_features
from mlops.homework03.utils.data_preparation.feature_selector import select_features
...
def test_dataset(
    ...
    *args,
    ...
    assert (
        X.shape[1] == 519
    ), f'Entire dataset should have 519 features, but has {X.shape[1]}'
    assert (
...
def test_training_set(
    ...
    *args,
    ...
    assert (
        X_train.shape[0] == 1743463
    ), f'Training set for training model should have 54378 examples, but has {X_train.shape[0]}'
    assert (
        X_train.shape[1] == 511
    ), f'Training set for training model should have 511 features, but has {X_train.shape[1]}'
    assert (
...
def test_validation_set(
    ...
    *args,
    ...
    assert (
        X_val.shape[0] == 1572753
    ), f'Training set for validation should have 51492 examples, but has {X_val.shape[0]}'
    assert (
        X_val.shape[1] == 511
    ), f'Training set for validation should have 511 features, but has {X_val.shape[1]}'
    assert (
